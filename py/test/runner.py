# Techy SDK test runner

from __future__ import annotations
import os
import json

from techy_sdk.utility.voxgig_struct import voxgig_struct as vs


class TechyTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            TechyTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = TechyTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = TechyTestRunner.getenv("TECHY_TEST_LIVE")
        override = TechyTestRunner.getenv("TECHY_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = TechyTestRunner.getenv(key)
        ...
        explain = TechyTestRunner.getenv("TECHY_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["TECHY_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if TechyTestRunner._test_control is not None:
            return TechyTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                TechyTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            TechyTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = TechyTestRunner.load_test_control()
...
def load_env_local():
    TechyTestRunner.load_env_local()


def env_override(m):
    return TechyTestRunner.env_override(m)


def entity_data(v):
    return TechyTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return TechyTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return TechyTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return TechyTestRunner.load_test_control()


def live_client_options():
    return TechyTestRunner.live_client_options()


def live_delay_ms():
    return TechyTestRunner.live_delay_ms()
