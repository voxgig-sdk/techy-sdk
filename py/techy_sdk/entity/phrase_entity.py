...
from techy_sdk.techy_types import (
    Phrase,
    PhraseLoadMatch,
)
...
class PhraseEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return PhraseEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Phrase:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Phrase().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
