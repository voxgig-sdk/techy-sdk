...
[MCP](https://modelcontextprotocol.io) server exposing the Techy SDK as
two agent tools — `techy_list` and `techy_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/techy-mcp)
make build

# 2. Provide credentials via the environment
export TECHY_APIKEY=sk_live_xxx
...
```jsonc
// techy_list: first page of records
{ "entity": "phrase" }
{ "entity": "phrase", "query": { } }

// techy_load: one record by id
{ "entity": "phrase", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/techy-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user techy \
     -- "$PWD"/dist/*/techy-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `techy_list` and `techy_load` tools now appear
   in new sessions. Ask the agent to *"list phrase using techy"*
   and it calls `techy_list` with `{"entity":"phrase"}`.
...
```sh
export TECHY_APIKEY=sk_live_xxx            # API key
export TECHY_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `techy_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `techy_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `TECHY_APIKEY` | API key sent with every request. |
| `TECHY_BASE` | Optional override of the API base URL. |
