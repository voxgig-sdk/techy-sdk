...
class FtCtrl extends TechyControl
{
...
class FtClient
{
    ...
    private TechyUtility $utility;

    public function __construct(TechyUtility $utility, array $options)
    {
    ...
    public function get_utility(): TechyUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (TechyContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (TechyContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = TechyConfig::make_feature($name);
    ...
    private static function buildUrl(TechySpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof TechyError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new TechyResponse($fetched);
        }
    ...
    private function fail(TechyContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (TechyContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
