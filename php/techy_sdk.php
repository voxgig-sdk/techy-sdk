...
class TechySDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = TechyHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = TechyHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!TechyFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, TechyFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return TechyUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = TechyHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = TechyHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = TechyHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new TechyError($op . "_allow",
                "TechySDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = TechyHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new TechyError("graphql_error",
                "TechySDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Phrase($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_phrase === null) {
                $this->_phrase = new PhraseEntity($this, null);
            }
            return $this->_phrase;
        }
