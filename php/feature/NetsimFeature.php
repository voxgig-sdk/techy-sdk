...
class TechyNetsimFeature extends TechyBaseFeature
{
    ...
    private function _simulate(TechyContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(TechyContext $ctx, array $applied): void
    {
        $client = $this->client;
