...
class PhraseEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new PhraseEntity($this->_client, $opts);
    }

    /**
     * @param Phrase|array $args Phrase data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = TechyHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Phrase|array The current Phrase data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Phrase fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = TechyHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Phrase fields).
     */
    ...
    /**
     * Load a single Phrase.
     *
     * @param PhraseLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed PhraseLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Phrase|array The loaded Phrase as an assoc-array at the
     *   SDK boundary; throws TechyError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = TechyHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
