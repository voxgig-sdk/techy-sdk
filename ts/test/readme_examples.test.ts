...
//     (\`new TechySDK(...)\` / \`TechySDK.test(...)\` become
//     \`TechySDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"phrase":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new TechySDK(...)\` and any
// \`TechySDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+TechySDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/TechySDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
