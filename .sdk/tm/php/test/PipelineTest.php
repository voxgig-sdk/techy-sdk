...
{
    private static function utility(): TechyUtility
    {
        return new TechyUtility();
    }
    ...
    private static function ctx(array $over = []): TechyContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new TechyContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new TechyOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof TechyError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new TechyOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = TechyMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new TechyOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = TechyMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new TechyOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = TechyMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TechySpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = TechyMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new TechyResponse([]),
            'result' => new TechyResult([]),
        ]);
        [, $err] = TechyMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new TechySpec(['step' => 's']),
            'response' => new TechyResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new TechyResult([]),
        ]);
        [, $err] = TechyMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new TechySpec(['step' => 's']),
            'response' => new TechyResponse(self::resp(200, ['v' => 1])),
            'result' => new TechyResult([]),
        ]);
        [, $err] = TechyMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new TechySpec(['step' => 's']),
            'response' => new TechyResponse(self::resp(200, ['v' => 2])),
            'result' => new TechyResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        TechyMakeResponse::call($ctx);
    ...
    {
        $preset = new TechyResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new TechySpec([]),
            'response' => new TechyResponse([]),
            'result' => new TechyResult([]),
        ...
        [$got, $err] = TechyMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new TechyResult([])]);
        [, $err] = TechyMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new TechySpec([]), 'result' => null]);
        [, $err] = TechyMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TechySpec(['step' => 's']),
            'result' => new TechyResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new TechyOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TechyMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TechySpec(['step' => 's']),
            'result' => new TechyResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new TechyOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TechyMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TechyResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new TechySpec([]),
            'result' => new TechyResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = TechyMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $utility = self::utility();
        $boom = new TechyError('boom', 'boom');
        $utility->fetcher = function (TechyContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TechySpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = TechyMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (TechyContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = TechyMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(TechyResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (TechyContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TechySpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = TechyMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new TechyResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new TechySpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = TechyMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = TechyMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = TechyMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new TechyResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, TechyDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new TechyResult(['ok' => false])]);
        $this->expectException(TechyError::class);
        TechyDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new TechyResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', TechyMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new TechyBaseFeature();
        $b = new TechyBaseFeature();
        TechyFeatureAdd::call($ctx, $a);
        TechyFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): TechyBaseFeature
    {
        $f = new TechyBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    private static function auth_ctx(array $options, ?array $headers): TechyContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new TechySpec(['headers' => $headers]),
        ]);
    }
    ...
    {
        $ctx = self::ctx([
            'response' => new TechyResponse(['headers' => null]),
            'result' => new TechyResult([]),
        ]);
        TechyResultHeaders::call($ctx);
