...
//    only - a typed TechyContext there would make every ctx
...
final class TechyOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new TechyContext($clean, null);
    ...
    public static function err_from_map($m): ?TechyError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new TechyError(is_string($code) ? $code : '', $msg);
    }
