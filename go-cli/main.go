// Command Techy-cli is a CLI + REPL for the Techy SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      TECHY_APIKEY (api key), TECHY_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: TECHY_APIKEY carries the API key and
	// TECHY_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("TECHY_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("TECHY_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewTechySDK(opts)
