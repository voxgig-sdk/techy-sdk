...
boru-driven command-line client **and** interactive REPL for the Techy
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/techy-cli)
make build
...
./techy-cli load 1 phrase            # {id:1} shorthand
./techy-cli load '{id:1}' phrase       # explicit match map

# 5. Override the API base URL for a single call
TECHY_BASE=https://api.example.com ./techy-cli load 1 phrase
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/techy-cli
   ```
...
```sh
./techy-cli load 1 phrase          # scalar shorthand for {id:1}
./techy-cli load '{id:1}' phrase     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `techy>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `phrase`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/techy-cli`. |
...
(`./techy-cli <expr>`) and interactively (the REPL), and expressions compose the
