...
import (
	...
	omni "github.com/voxgig-sdk/techy-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.TechyError); is {
				out := map[string]any{
					"name":    "TechyError",
...
func omniCtx(arg any, client *sdk.TechySDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.TechySDK); is {
			client = live
