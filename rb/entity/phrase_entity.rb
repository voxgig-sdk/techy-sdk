...
class PhraseEntity
  ...
  def make
    opts = @_entopts.dup
    PhraseEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = TechyHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Phrase, Hash] the current Phrase data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = TechyHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Phrase fields)
  def match_get
  ...
  #
  # @param reqmatch [PhraseLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Phrase.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Phrase, Hash] the loaded Phrase; raises TechyError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = TechyHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
