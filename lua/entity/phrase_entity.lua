...
local PhraseEntity = {}
PhraseEntity.__index = PhraseEntity


function PhraseEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PhraseEntity)
  self._name = "phrase"
  self._client = client
...
function PhraseEntity:get_name()
  return self._name
end


function PhraseEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PhraseEntity.new(self._client, opts)
end
...
function PhraseEntity:mark_deleted()
  self._deleted = true
end


function PhraseEntity:deleted()
  return true == self._deleted
end


function PhraseEntity:data_set(args)
  if args ~= nil then
...
function PhraseEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PhraseEntity:match_set(args)
  if args ~= nil then
...
function PhraseEntity:match_get()
...
function PhraseEntity:stream(action, args, callopts)
...
---@param reqmatch PhraseLoadMatch
---@param ctrl? table
---@return Phrase
---@return string? err
function PhraseEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function PhraseEntity:_run_op(ctx, post_done)
  local utility = self._utility
